...
```{r}
# In your .qmd header for this chunk:
...
# Add 20% headroom so bars aren't squashed
injury_max <- max(injury_summary$total, na.rm = TRUE) * 1.20
attack_max <- max(attack_summary$total, na.rm = TRUE) * 1.20

make_plot <- function(data, hosp, ymax) {
  ggplot(
    ...
    labs(x = NULL, y = "Total Count", title = hosp) +
    better_theme
}
...
inj_row <- (
  make_plot(injury_summary, hospital_levels[1], injury_max) |
  make_plot(injury_summary, hospital_levels[2], injury_max) |
  make_plot(injury_summary, hospital_levels[3], injury_max)
)

att_row <- (
  make_plot(attack_summary, hospital_levels[1], attack_max) |
  make_plot(attack_summary, hospital_levels[2], attack_max) |
  make_plot(attack_summary, hospital_levels[3], attack_max)
)

final_plot <- inj_row /
              att_row +
  plot_annotation(
    title = "Hospital-Level Distribution of Injuries and Attacks",
    theme = theme(
